...
FIXES APPLIED:
  [C3] KPI labels say "Est. Churn Rate (heuristic)"; info banner explains method.
  [W4] Donor retention forecast runs ARIMA on actual donor counts directly.
"""
...
def render():
    page_header("👥 Donor Analytics", "Churn estimation, retention forecasting, and donor growth intelligence")
    ...
    st.info(
        "ℹ️ **Churn estimation method:** Month-over-month donor count changes "
        f"({churn.get('method', 'heuristic')}). "
        "For a trained classifier, replace `predict_donor_churn()` in `predict.py` "
        "with a model trained on your labelled donor records."
    )

    # ── KPI row ───────────────────────────────────
    c1, c2, c3, c4 = st.columns(4)
    for col, label, val, icon in [
        (c1, "Est. Churn Rate",     f"{churn['churn_rate']}%",      "🔴"),
        (c2, "Est. Retention Rate", f"{churn['retention_rate']}%",  "💚"),
    ...
    st.markdown("#### 📅 Donor Count Forecast (Next 12 Weeks)")

    if "donors" in df.columns:
        # [W4-FIX] ARIMA on actual donor counts, not scaled amounts
        donor_weekly = (
            df[["date", "donors"]]
            .copy()
            .assign(week_start=lambda d: d["date"] - pd.to_timedelta(d["date"].dt.dayofweek, unit="d"))
            .groupby("week_start")["donors"]
            .sum()
            .reset_index()
            .rename(columns={"week_start": "date", "donors": "amount"})
        )

        if len(donor_weekly) >= 6:
            fc_donors, _ = forecast_arima(donor_weekly, horizon=12, freq="W")
            ...
            fig.update_yaxes(gridcolor=GRID, tickfont=dict(color=MUTED), title_text="Donor Count")
            fig.update_xaxes(gridcolor=GRID, tickfont=dict(color=MUTED))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Need at least 6 weekly data points to forecast donor counts.")
    else:
        st.info(
            "No `donors` column found. Upload data with a `donors` column "
            "to see a donor count forecast."
        )
    ...
    recs = [
        ("Send personalised thank-you emails within 48h of each donation",
         "Boosts repeat donation rate by ~20%"),
        ("Launch re-engagement campaign targeting donors inactive for 60+ days",
         f"~{churn['at_risk_donors']:,} donors qualify"),
        ("Introduce recurring donation option with incentive",
         "Recurring donors have 5× higher LTV"),
        ("Monthly impact reports to existing donors",
         "Increases donor retention by 15–25%"),
    ]
