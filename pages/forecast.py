...
MODEL_DESC = {
    ...
    "LSTM":    "Deep learning. Best for complex long-term sequences.",
}
...
def render():
    ...
    if "forecast_df" not in st.session_state or run_btn:
        progress_bar = st.progress(0, text=f"Initialising {model_name}…")
        import time
        for pct, msg in [
            (20, f"Preprocessing data…"),
            (45, f"Fitting {model_name} model…"),
            (75, f"Generating {horizon}-period forecast…"),
            (95, f"Computing confidence intervals…"),
        ]:
            time.sleep(0.18)
        ...
        progress_bar.progress(100, text="✅ Forecast complete!")
        time.sleep(0.3)
        progress_bar.empty()
