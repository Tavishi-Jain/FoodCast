...
def render():
    ...
    # FIX #7 — disclose that the model is trained on synthetic data
    st.info(
        "ℹ️ **About this predictor:** The Random Forest model is currently trained on synthetic "
        "data. Predictions give directional guidance based on heuristics (budget vs historical "
        "average, season, duration). For production use, retrain the model on your own labelled "
        "campaign outcomes — replace the training data in `predict.py → predict_campaign_success()`."
    )
    ...
    if predict_btn or "campaign_result" in st.session_state:
        ...
        summary_data = {
            ...
            "Value":     [format_currency(budget), f"{duration} days",
