...
FIXES APPLIED:
  [C2]   Multi-feature synthetic label; updated disclosure banner.
  [UX-3] Stale results no longer shown on page load — only after button click.
"""
...
def render():
    page_header("🎯 Campaign Success Predictor",
                "Random Forest model to forecast campaign outcomes before you launch")

    st.info(
        "ℹ️ **About this predictor:** The Random Forest model is trained on **synthetic data** — "
        "500 simulated campaigns with labels generated from a multi-feature scoring function "
        "(budget vs historical average, duration, festival season, donor count, past experience). "
        "Predictions are directional guidance only. "
        "For production use, replace the training data in `predict.py → predict_campaign_success()` "
        "with your own labelled campaign outcomes from a CRM or fundraising platform."
    )
    ...
    with col1:
        budget         = st.number_input("Campaign Budget (₹)", min_value=1_000.0,
                                         max_value=10_000_000.0,
                                         value=default_budget, step=5_000.0, format="%.0f")
    ...
    with col2:
        hist_avg       = st.number_input("Historical Monthly Average (₹)", min_value=1_000.0,
                                         max_value=5_000_000.0,
                                         value=default_budget * 0.9, step=5_000.0)
        past_campaigns = st.slider("Past Campaigns Run", 0, 50, 5)
        avg_donors     = st.number_input("Expected Donors", min_value=1.0,
                                         max_value=10_000.0,
    ...
    # [UX-3 FIX] Only show results after explicit button click
    if predict_btn:
    ...
    if "campaign_result" not in st.session_state:
        ...
        <div style="margin-top:10px">Configure your campaign above and click Predict</div>
        </div>
    ...
    with col_r:
        ...
        factors = [
            ("Budget vs Historical",
             "✅ Strong" if budget > hist_avg else "⚠️ Below average",
             budget > hist_avg),
            ("Campaign Duration",
             "✅ Sufficient" if duration >= 14 else "⚠️ Short",
             duration >= 14),
            ("Festival Season",
             "✅ Peak window" if month in [10, 11, 12, 3] else "🔵 Off-peak",
             month in [10, 11, 12, 3]),
            ("Experience Level",
             "✅ Experienced" if past_campaigns >= 3 else "🆕 New",
             past_campaigns >= 3),
            ("Donor Engagement",
             "✅ Good" if avg_donors >= 50 else "⚠️ Low",
             avg_donors >= 50),
        ]
    ...
    summary_data = {
        "Parameter": ["Budget", "Duration", "Launch Month", "Historical Avg",
                      "Past Campaigns", "Expected Donors"],
        "Value": [
            format_currency(budget), f"{duration} days",
            datetime(2024, month, 1).strftime("%B"),
            format_currency(hist_avg), str(past_campaigns), f"{avg_donors:.0f}"
        ],
        "Status": [
            "✅" if budget >= hist_avg else "⚠️",
            "✅" if duration >= 14 else "⚠️",
            "🔥" if month in [10, 11, 12, 3] else "🔵",
            "—",
            "✅" if past_campaigns >= 3 else "🆕",
            "✅" if avg_donors >= 50 else "⚠️"
        ]
    }
