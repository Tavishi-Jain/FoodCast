...
# FIX #11 — Currency symbols for international users
CURRENCY_OPTIONS = {
    "₹  Indian Rupee (INR)":   "₹",
    "$  US Dollar (USD)":      "$",
    "€  Euro (EUR)":           "€",
...
with st.sidebar:
    ...
    # FIX #11 — Currency selector
    currency_label = st.selectbox(
        "💱 Currency",
        options=list(CURRENCY_OPTIONS.keys()),
        index=0,
        help="Select your organisation's currency. All values will display with this symbol.",
    )
    ...
    <div style="font-size:0.72rem;color:rgba(255,255,255,0.3);line-height:1.6">
      ...
      <div style="margin-top:4px">v1.1.0 · Built for NGOs & CSR Teams</div>
    </div>
    """, unsafe_allow_html=True)
...
def _render_landing():
    ...
    <div style="text-align:center;padding:48px 0 24px;color:rgba(255,255,255,0.25);font-size:0.78rem">
      Built with ❤️ for social impact · FoodCast v1.1.0 · Hugging Face Spaces Ready
    </div>
    """, unsafe_allow_html=True)
